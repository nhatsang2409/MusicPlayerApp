...
class UploadActivity : AppCompatActivity(), MusicAdapter.OnMusicItemClickListener {
    ...
    private fun initializeCloudinary() {
        Executors.newSingleThreadExecutor().execute {
            try {
                val config = mapOf(
                    "cloud_name" to "dahc6d65l",
                    "api_key" to "826316947957284",
                    "api_secret" to "RFrHsgFM2As2qyEuhJLZ_-AgvIU"
                )
    ...
    private fun saveToFirebaseAndLocal(cloudinaryUrl: String) {
        ...
        val ref = FirebaseDatabase.getInstance(databaseUrl).getReference("uploads").child(currentUser.uid)
